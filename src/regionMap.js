...
export const stateFiscalHealth = {
  // Low debt = green (#2D5A27), Med = amber (#B8860B), High = red (#8B0000)
  'MH': 'medium', 'GJ': 'low', 'KA': 'medium', 'TN': 'high', 'UP': 'high',
  'DL': 'low', 'PB': 'high', 'BR': 'high', 'WB': 'high', 'RJ': 'high',
  'MP': 'medium', 'AP': 'high', 'TG': 'medium', 'KL': 'high', 'HR': 'medium'
  // Defaults to 'medium' if not mapped
};

export const getFiscalColor = (healthLevel) => {
  if (healthLevel === 'low') return 'var(--accent-green)';
  if (healthLevel === 'high') return 'var(--accent-red)';
  return 'var(--accent-gold)'; // medium
};
